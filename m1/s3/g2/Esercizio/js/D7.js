...
console.log(conc('epicode', 'developer'))
...
let numeriDispari = () => {
  ...
  return numeri.filter((num) => num % 2 !== 0);
...
let numeriCinque = () => {
  ...
  return numeri.filter((num) => num % 5 === 0);

}

console.log(numeriCinque());
...
let numeriCasuali = () => {
  ...
  for (i = 0; i < 10; i++) {
    n.push(Math.floor(Math.random(i) * 101))
  }
  return n
}
...
let numbers = [6, 43, 71, 472, 58, 27];

let plusOne = numbers.map(a => a + 1)
...
let stringLength = ['Epicode', 'web', 'developer']

let stringModification = () => {
  let lenghts = stringLength.map(item => item.length)
  return lenghts
}
...
let numeros = [6, 43, 71, 472, 58, 27];

let evenDeletion = () => {
  let numero = numeros.filter((a) => a % 2 != 0)
  return numero
}

console.log(evenDeletion());
